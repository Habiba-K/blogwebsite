        ...
        <div className={`text-white ${navOpen} transform transition-all duration-500 delay-300 fixed justify-center flex flex-col h-full w-[80%] sm:w-[60%] bg-[#0f0715] space-y-6 z-[10000]`}>
                {navLinks.map((navlink)=>{
                    return <Link key={navlink.id} href={navlink.url}>
                        <p className="relative text-base font-medium w-fit block after:block after:content-[''] after:absolute after:h-[3px] after:bg-blue-400 text-white after:w-full after:scale-x-0 after:hover:scale-100 after:transition after:duration-300 after:origin-right text-[20px] ml-12 border-b-[1.5px] pb-2 border-white sm-text-[30px]">{navlink.label}</p>
                    </Link>
                })}
        {/*close button */}
